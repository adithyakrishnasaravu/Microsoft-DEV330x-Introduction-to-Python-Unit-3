...
def parse_command_line():
    ...
    optional arguments:
      ...
      -o outfile_path, --outfile outfile_path
                            output file
      -k KEY, --key KEY     encryption/decryption key (must be positive) (default= 1)
    ...
    parser.add_argument('-k', '--key', action='store', type=int, default=1,
                        help='encryption/decryption key (must be positive) (default= 1)')

    parser.add_argument('-d', '--decrypt', action='store_true', help='decrypt the input file')

    parser.add_argument('-a', '--all', action='store', type=str,
                        help='decrypt using all keys [1, 25], save outputs in/n different files. (useful in case the key is lost or/n unknown)')
...
def transform(message, key, decrypt):
    ...
    if decrypt == True:
        for i in message:
            transformed_message += shift(i, -1 * key)
        return transformed_message
    else:
        for i in message:
            transformed_message += shift(i, key)
        return transformed_message
...
def main():
    ...
    instring = read_file('/home/nbuser/library/' + args.infile)
    # verbose
    if args.verbose:
        print("Input file:")
        print("------------")
    ...
    if not args.all:
        ...
        write_file(outstring, '/home/nbuser/library/' + args.o)
